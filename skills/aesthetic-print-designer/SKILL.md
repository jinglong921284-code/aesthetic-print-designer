...
6. Keep the sheet at `Draft`, `Design alignment`, or `Revise` status until its own evidence supports another design-stage status. Do not run the closure state machine, generate a garment mockup, write to Feishu, contact a supplier, send a file, or enter production handoff unless the user separately asks and authorizes that action.
