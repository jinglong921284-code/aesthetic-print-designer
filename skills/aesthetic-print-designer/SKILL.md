...
- For a standalone request such as **生成规格单、出印花规格单、Print Specification Sheet、印花设计规格单**, read `references/pantone-spec-sheet.md` and use `assets/print-spec-sheet-template.md`. Start the independent **Print Specification Sheet / 印花规格单** output mode for designer, merchandising, or pattern-room alignment. Do not start tracked closure, supplier handoff, an external/cloud-document write, or a production-approval workflow unless the user separately requests and authorizes it.
- For **视觉版规格单、英文规格单、两页规格单、visual print spec sheet**, also read `references/visual-print-spec-sheet.md`. Choose colour-focused or full technical scope within Print Specification Sheet mode, then choose the presentation format. The bundled English two-page PDF preset is a format, not another business stage or approval gate.
- Numbered artwork, colour tables, specification documents, supplier use, and production work follow `references/pantone-spec-sheet.md`. New extraction uses the role-locked `colour-spec` path; translation or reformatting preserves the existing mapping and provenance. The quick route must not be promoted into a specification.
...
14. For colour work beyond the lightweight exact-HEX quick route—or whenever the task includes numbered colour annotation or a specification document—read `references/pantone-spec-sheet.md` before editing images or documents. For **Print Specification Sheet / 印花规格单** mode, start from `assets/print-spec-sheet-template.md`, preserve the artwork exactly, map every numbered callout to exactly one table row in the same order, keep physical size in the specification field rather than drawing dimension arrows on the artwork, and distinguish computed matches from screen-only candidates.
...
## Print Specification Sheet Fast Path

This is the explicit one-pass path for a request such as **选中这个印花后生成规格单**. It creates a design-stage document for designer, merchandising, and pattern-room alignment; it is not a supplier handoff or approval workflow.

1. Identify the selected artwork and reuse its approved Print ID, name, source filename, and version. If an ID, name, or value is missing, write `待确认`; never invent a production identifier.
2. Lock the source non-destructively by recording its filename, version, and checksum when the user has authorized a local output location. If no persistent output is authorized, retain the original source reference and mark the persistent lock `待确认`.
3. For new colour extraction, lock the intended colour roles from the full-resolution source in a copy of `assets/colour-role-template.json`. Add, remove, and renumber roles so their sequential IDs match the actual intended colour count; the starter roles do not impose a three-colour limit. Then run `python3 scripts/run_print_tool.py colour-spec` with an authorized named-colour library. Without one, retain source HEX values and mark the named-colour candidate `待匹配`; do not fabricate Pantone data. For translation or reformatting of an existing specification, reuse its unchanged numbered image and colour rows, preserve their source/version and stated limitations, and do not resample or recalculate unless requested. An embedded annotation image is not the locked production master.
4. Record repeat evidence only for a repeat architecture. For placement, border, engineered, or panel artwork, fill the applicable boundary, scale, orientation, named zone or pattern-piece fields instead of forcing repeat validation.
5. Fill the relevant scope in `assets/print-spec-sheet-template.md`, retaining unknown technical fields as pending. Separate representative colour count from production separation count. Return the sheet with the annotation and one-to-one colour table in the requested local format. For the visual English PDF preset, follow `references/visual-print-spec-sheet.md`; preserve the user's artwork-use notice and inspect every rendered page before delivery.
6. Use `Draft / Design Alignment / Ready for Sampling / Revise` as defined in `references/pantone-spec-sheet.md`. Set `Ready for Sampling` when design, merchandising, and pattern-room confirmations are recorded for the current version and no unresolved issue blocks sampling. Physical colour and actual-fabric review may remain pending for the sampling stage. Do not run the closure state machine, generate a garment mockup, write to Feishu, contact a supplier, send a file, or enter production handoff unless the user separately asks and authorizes that action.

If the artwork has not been explicitly selected, the same mode may produce a `Draft` sheet with the source marked `candidate`. Creating a specification never promotes an artwork to selected, physically approved, or production approved.
...
Use **Print Specification Sheet / 印花规格单** mode above when the user asks only for a specification sheet. Read `references/selection-closure-routing.md` only when the user asks to start tracked closure, prepare sampling materials, hand off the design, or has already enabled a named automatic-closure mode. Resolve an approved project directory, existing or user-confirmed Print ID and design name, exact source bytes, print architecture, and the allowed local/external actions. Never invent a production identifier or initialize from a screenshot or re-encoded substitute.
...
- **Print Specification Sheet / 印花规格单:** choose **Colour Specification / 配色专项版** for numbered artwork, representative colours, digital candidates, and colour notes; or **Full Technical Specification / 完整技术版** to include application, scale, repeat/placement, material, process, file technicals, source control, version, status, and open fields. Choose Markdown, editable DOCX through available document tools, or the bundled English visual PDF preset separately. Both scopes support design-stage alignment, not supplier transfer or production approval.
