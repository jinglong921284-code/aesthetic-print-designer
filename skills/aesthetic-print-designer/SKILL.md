...
3. Extract and lock the intended colour roles from the full-resolution source in a copy of `assets/colour-role-template.json`. Add, remove, and renumber roles so their sequential IDs match the actual intended colour count; the starter roles do not impose a three-colour limit. Then run `python3 scripts/run_print_tool.py colour-spec` with an authorized named-colour library. It produces the role-locked colour report and numbered annotation. Without an authorized library, retain the source HEX values and mark the named-colour candidate `待匹配`; do not fabricate Pantone data.
...
5. Fill every field in `assets/print-spec-sheet-template.md`, including colour count, Pantone or other named-system candidate, material, process, resolution, colour mode, technical notes, status, and unresolved fields. Return the filled sheet with the annotation and colour table in the requested or authorized local format.
...
- **Print Specification Sheet / 印花规格单:** a standardized design-stage sheet containing source control, Print ID and name, type, size, repeat or placement settings, colour count and annotated HEX/named-colour candidates, fabric, process, resolution, colour mode, version, status, technical notes, and open fields. It supports designer, merchandising, and pattern-room alignment; it is not a supplier transfer or production approval.
